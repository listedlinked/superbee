...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "solaris.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "solarisd.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, solaris, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Solaris specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzpiv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zXLR minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizesolarisamount=<n>     " + strprintf(_("Keep N XLR anonymized (default: %u)"), 0) + "\n";
...
/** Sanity checks
 *  Ensure that Solaris is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Solaris Core is shutting down."));
...
#endif
    // Make sure only a single Solaris process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Solaris Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Solaris: load previous sessions sporks if we have them.
                ...
                // Solaris: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Solaris Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Solaris Core to complete") << "\n";
...
//    }
//
//    nAnonymizeSolarisAmount = GetArg("-anonymizesolarisamount", 0);
//    if (nAnonymizeSolarisAmount > 999999) nAnonymizeSolarisAmount = 999999;
//    if (nAnonymizeSolarisAmount < 2) nAnonymizeSolarisAmount = 2;
    ...
    LogPrintf("Anonymize Solaris Amount %d\n", nAnonymizeSolarisAmount);
