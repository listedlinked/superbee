...
#if defined(NDEBUG)
#error "Superbee cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 2.75 * COIN;
    } else if (nHeight < 80 && nHeight >= 50) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 80) {
        nSubsidy = 2 * COIN;
    } else if (nHeight <= 110 && nHeight > Params().LAST_POW_BLOCK()) {
        nSubsidy = 1 * COIN;
    } else if (nHeight <= 200 && nHeight >= 110) {
        nSubsidy = 0.95 * COIN;
    } else if (nHeight <= 250 && nHeight >= 200) {
        nSubsidy = 0.90 * COIN;
    } else if (nHeight <= 300 && nHeight >= 250) {
        nSubsidy = 0.85 * COIN;
    } else if (nHeight <= 350 && nHeight >= 300) {
        nSubsidy = 0.80 * COIN;
    } else if (nHeight <= 400 && nHeight >= 350) {
        nSubsidy = 0.75 * COIN;
    } else if (nHeight <= 450 && nHeight >= 400) {
        nSubsidy = 0.70 * COIN;
    } else if (nHeight <= 500 && nHeight >= 450) {
        nSubsidy = 0.75 * COIN;
    } else if (nHeight <= 550 && nHeight >= 500) {
        nSubsidy = 0.70 * COIN;
    } else if (nHeight >= 550) {
        nSubsidy = 0.25 * COIN;
    } else {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Superbee
...
{
    RenameThread("superbee-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Superbee: We use certain sporks during IBD, so check to see if they are
