...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSuperbeeAmount"))
        settings.setValue("nAnonymizeSuperbeeAmount", 1000);

    nAnonymizeSuperbeeAmount = settings.value("nAnonymizeSuperbeeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSuperbeeAmount"))
        SoftSetArg("-anonymizesuperbeeamount", settings.value("nAnonymizeSuperbeeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in superbee.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSuperbeeAmount:
            return QVariant(nAnonymizeSuperbeeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSuperbeeAmount:
            nAnonymizeSuperbeeAmount = value.toInt();
            settings.setValue("nAnonymizeSuperbeeAmount", nAnonymizeSuperbeeAmount);
            emit anonymizeSuperbeeAmountChanged(nAnonymizeSuperbeeAmount);
            break;
        case CoinControlFeatures:
