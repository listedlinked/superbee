...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Superbee address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no Superbee: URI
...
{
    // Convert superbee:// to superbee:
    //
    //    Cannot handle this later, because superbee:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open superbee.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Superbee.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Superbee.lnk
...
{
    return GetAutostartDir() / "superbee.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a superbee.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Superbee\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the superbee app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add superbee app to startup item list
